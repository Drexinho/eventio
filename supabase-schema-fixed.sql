...
-- Events: Allow INSERT for anyone (creating events), SELECT/UPDATE/DELETE for token holders
...
CREATE POLICY "Allow update by token" ON events
  FOR UPDATE USING (
    access_token = current_setting('app.access_token', true)::text
  );

CREATE POLICY "Allow delete by token" ON events
  FOR DELETE USING (
    access_token = current_setting('app.access_token', true)::text
  );
